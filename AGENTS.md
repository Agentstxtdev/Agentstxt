...
2. **Demonstrates the spec live** by serving its own `/agents.txt`, `/agents.json`, plus four ecosystem discovery surfaces (`/.well-known/api-catalog`, `/.well-known/mcp/server-card.json`, `/.well-known/agent-skills/index.json`, `/openapi.json`) from `public/`. All discovery files are generated by `herald emit` from `agentsjson.config.js`. See [`CLAUDE.md`](CLAUDE.md) "the user adds a new well-known path" for the full table.
