# AGENTS.md: agents-txt repository
...
2. **A live deployment** at [agents-txt.com](https://agents-txt.com) that serves the spec, hosts demos, runs an MCP server, and an agent-auth worker. Three separate Cloudflare Workers, one Astro site.
...
2. **Demonstrates the spec live** by serving its own `/agents.txt`, `/agents.json`, plus four ecosystem discovery surfaces (`/.well-known/api-catalog`, `/.well-known/mcp/server-card.json`, `/.well-known/agent-skills/index.json`, `/openapi.json`) and the public JSON Schema at `/schema/agents-json/v1.0.json` from `public/`. The four ecosystem files are generated by `herald emit` from `agentsjson.config.js`; the JSON Schema is generated by `@agentstxtdev/herald-schema` from its Zod source and committed here. Every emitted `agents.json` carries `"$schema": "https://agents-txt.com/schema/agents-json/v1.0.json"` at the top so editors offer autocomplete and inline validation on hand-edited files. See [`CLAUDE.md`](CLAUDE.md) "the user adds a new well-known path" for the full table.
3. **Acts as a Backend-for-Frontend (BFF)** via `src/worker.ts`: proxies `/mcp` to the MCP worker, `/.well-known/agent-configuration` and `/agent/*` to the auth worker, self-serves two synthetic gated routes (`/x402` for x402 v2 on Solana and `/mpp` for MPP via `mppx`), and handles `Accept: text/markdown` content negotiation on HTML page paths by returning `/llms-full.txt` with the matching content type.
...
A Cloudflare Worker exposing the `agents.txt` spec to AI agents over Model Context Protocol. Tools: `get_spec`, `parse_agents_txt`, `validate_agents_txt`, `validate_agents_json`, `audit_site`, `get_skill`. Built on `hono`, `@modelcontextprotocol/sdk`, `agents`. Deploys to `mcp.agents-txt.com`.

`audit_site` does the heavy lifting: it fetches `/agents.txt`, `/agents.json`, and `/robots.txt`, validates the §4.5 serving headers (Content-Type, CORS, Cache-Control), runs the §3, §6–§11 directive validators on `agents.txt`, schema-validates `agents.json` per §5, scans both files for accidental treasury / secret leaks per §5.4 / §14, and cross-checks that the two files declare consistent capabilities (payments / authorization / MCP / Skills / A2A). Out of scope: full RFC 9309 audit, sitemap.xml, llms.txt. Those are governed by other specs.
...
`validate_agents_json` returns a four-field `ValidationResult`: `valid`, `errors`, `warnings`, and `notes`. The `notes` field is the positive-observation channel; a recognised `$schema` reference at the top of a served `agents.json` lands there with a `Schema reference present: <url>` entry. Absence of `$schema` produces a warning that includes the canonical hosted URL. The validator does not fetch the referenced schema; presence and string-shape are enough at this layer. Consumers that want full JSON Schema conformance run `AgentsJsonSchema.safeParse(json)` from `@agentstxtdev/herald-schema` instead.
...
- No npm-publishable packages. Nothing in `agents-txt/` is published.
...
- **Follow existing code style.** No Prettier/Biome config inside `agents-txt/` yet; match the formatting you see.
...
Use this skill when a user is working in *their own* repository and asks how to make their site agent-readable. It is **not** for working in this repo (the agents-txt monorepo itself).
...
| Adjust the Markdown-for-Agents page-path allowlist | `site/src/worker.ts` (`PAGE_PATHS` regex; covers `/`, `/spec`, `/demo`, `/demo/<name>`) |
| Update the MCP server card metadata | `site/agentsjson.config.js` `mcp.serverCard` (then `herald emit`) |
| Update the agent-skills discovery index entry | `site/agentsjson.config.js` `skills.urls` (regenerate `digest` via `sha256sum public/skills/<name>/SKILL.md`) |
| Update the OpenAPI payable-paths declaration | `site/agentsjson.config.js` `payments.openapi.paths` (then `herald emit`) |
...
| Adjust top-level scripts | `package.json` (root of `agents-txt/`) |
...
### 2. Register an identifier in an existing block (§8 Payments or §11 Authorization)

The protocol fits the existing semantics of an existing block and is stable enough to register. Steps:

1. Open a PR against [`spec/AGENTS-TXT-STANDARD.md`](spec/AGENTS-TXT-STANDARD.md). Add a subsection to §8 or §11 describing what the identifier signals to an agent and where the protocol's own details live (well-known path, response challenge, SDK).
...
4. If the protocol carries structured fields in `agents.json` (chains, methods, etc.), document the per-protocol object shape in §5.2 and §5.3, then add a per-protocol JSON shape check in [`mcp/src/tools/validate_agents.ts`](mcp/src/tools/validate_agents.ts) and [`audit_site.ts`](mcp/src/tools/audit_site.ts) alongside the existing x402 / MPP checks.
...
3. **`agents.json` schema** entry in §5.2, with a field-notes paragraph in §5.3. For URL-carrying blocks, mirror the `mcp[]` / `skills[]` shape: array of `{ url, description? }`, description is `agents.json`-only because the announcement layer (`agents.txt`) stays terse.
4. **Reference deployment**:
   ...
   - Add the §N directive check and §5.2 array check in [`mcp/src/tools/audit_site.ts`](mcp/src/tools/audit_site.ts), plus the cross-file consistency check that the URL set in `agents.txt` equals the URL set in `agents.json`.
5. **Renumbering**. If the new block is inserted before any existing section (the A2A change inserted between §7 Skills and the previous §9 Relationship), renumber the subsequent sections and update every `§N` reference in `audit_site.ts`'s rule messages. Search the audit code for `§` to find them all.
