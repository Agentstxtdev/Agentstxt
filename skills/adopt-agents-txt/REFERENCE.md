...
| Express / Hono / Next.js handlers | Set in the route handler | n/a — `@agentify/web` middleware does this |

Adopters using `agentify` on a supported platform get the file emitted automatically (default mode of `agentify generate`, or `agentify generate --headers` to emit only the headers config). Other platforms require manual configuration with the values above.
